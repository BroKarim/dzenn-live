...
import { type ComponentProps, Fragment, type HTMLAttributes, type ReactElement, type ReactNode, type RefObject, useEffect, useRef, useState } from "react";
    ...
    <>
      {showShaders && (
        <GrainGradient
          ...
          colors={resolvedTheme === "dark" ? ["#39BE1C", "#9c2f05", "#7A2A0000"] : ["#fcfc51", "#ffa057", "#7A2A0020"]}
          colorBack="#00000000"
          softness={1}
          intensity={0.9}
          noise={0.5}
      ...
      {/* <Image
        ...
        src={HeroImage}
        alt="hero-image"
        className={cn("absolute top-[460px] left-[20%] max-w-[1200px] rounded-xl border-2 lg:top-[400px]", imageReady ? "animate-in fade-in duration-400" : "invisible")}
...
function LaunchAppWindow(props: HTMLAttributes<HTMLDivElement>) {
  return (
    <div {...props} className={cn("overflow-hidden rounded-md border bg-fd-popover shadow-lg", props.className)}>
      <p className="text-xs text-fd-muted-foreground text-center px-4 py-2 border-b">localhost:3000</p>
...
//         />
//         {previews.map((item, i) => (
//           <button key={i} className={cn(previewButtonVariants({ active: active === i }))} onClick={() => setActive(i)}>
//             {item.name}
//           </button>
//         ))}
//       </div>
//       {previews.map((item, i) => (
//         <Image key={i} src={item.image} alt="preview" className={cn("col-start-1 row-start-1 select-none", active === i ? "animate-in fade-in slide-in-from-bottom-12 duration-800" : "invisible")} />
//       ))}
//     </div>
//   );
// }
...
export function Writing({ tabs: tabContents }: { tabs: Record<(typeof WritingTabs)[number]["value"], ReactNode> }) {
  const [tab, setTab] = useState<(typeof WritingTabs)[number]["value"]>("writer");

  return (
    <div className="col-span-full my-20">
      <h2 className="text-4xl text-brand mb-8 text-center font-medium tracking-tight">Anybody can write.</h2>
      <p className="text-center mb-8 mx-auto w-full max-w-[800px]">Native support for Markdown & MDX, offering intuitive, convenient and extensive syntax for non-dev writers, developers, and AI agents.</p>
      <div className="flex justify-center items-center gap-4 text-fd-muted-foreground mb-6">
        {WritingTabs.map((item) => (
          <Fragment key={item.value}>
            ...
            <button className={cn("text-lg font-medium transition-colors", item.value === tab && "text-brand")} onClick={() => setTab(item.value)}>
              {item.name}
            </button>
          </Fragment>
        ))}
...
export function AgnosticBackground() {
  ...
  return (
    <div ref={ref} className="absolute inset-0 -z-1 mask-[linear-gradient(to_top,white_30%,transparent_calc(100%-120px))]">
      <Dithering colorBack="#00000000" colorFront="#c6bb58" shape="warp" type="4x4" speed={visible ? 0.4 : 0} className="size-full" minPixelRatio={1} />
    </div>
  );
}
